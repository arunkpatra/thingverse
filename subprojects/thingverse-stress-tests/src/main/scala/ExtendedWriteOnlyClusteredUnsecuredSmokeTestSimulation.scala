...
class ExtendedWriteOnlyClusteredUnsecuredSmokeTestSimulation extends Simulation {
  ...
  val closedModelTrafficProfile = Seq(
    ...
    constantConcurrentUsers(256) during (15 minutes) // Welcome to Akka - You are cruising at 36,000 feet.
  )

  val basicScenarioUnSecuredSmokeTest: ScenarioBuilder = scenario("Extended Test Unsecured Smoke Test")
  ...
  setUp(basicScenarioUnSecuredSmokeTest.inject(closedModelTrafficProfile).protocols(httpProtocolStandalone))
}
